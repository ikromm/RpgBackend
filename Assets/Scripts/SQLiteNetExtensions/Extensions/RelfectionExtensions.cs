...
namespace SQLiteNetExtensions.Extensions
{
    ...
    public static class ReflectionExtensions
    {
        ...
        public static Type GetEntityType(this PropertyInfo property, out EnclosedType enclosedType)
        {
            ...
            else if (type.IsGenericType && typeof(ObservedList<>).IsAssignableFrom(type.GetGenericTypeDefinition()))
            {
                type = type.GetGenericArguments()[0];
                enclosedType = EnclosedType.ObservedList;
            }
            return type;
        }
        ...
        public static PropertyInfo GetInverseProperty(this Type elementType, PropertyInfo property)
        {
            ...
            {
                var properties = (from p in propertyType.GetProperties() where p.IsPublicInstance() select p);
                foreach (var inverseProperty in properties)
                {
                    ...
                    if (inverseAttribute != null && inverseType.Equals(elementType))
                    {
        ...
        private static bool IsPublicInstance(this PropertyInfo propertyInfo)
        {
            return propertyInfo != null &&
                   ((propertyInfo.GetGetMethod() != null && !propertyInfo.GetGetMethod().IsStatic && propertyInfo.GetGetMethod().IsPublic) &&
                   (propertyInfo.GetSetMethod() != null && !propertyInfo.GetSetMethod().IsStatic && propertyInfo.GetSetMethod().IsPublic));
        }
    }
}
